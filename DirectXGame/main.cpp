...
#pragma region 被写体の準備

#pragma region Cube

	// 被写体の準備
	Model* model = Model::CreateFromOBJ("cube");

	WorldTransformEx worldTransform;
	worldTransform.Initialize();
	worldTransform.translation_ = Vector3(0.0f, 0.5f, 0.0f);
...
#pragma region Terrain
	...
	// 被写体の準備
	Model* M_terrain = Model::CreateFromOBJ("terrain");

	WorldTransformEx WT_terrain;
	WT_terrain.Initialize();
	WT_terrain.translation_ = Vector3(0.0f, 0.0f, 0.0f);
	WT_terrain.scale_ = Vector3(2.0f, 1.0f, 2.0f);
...
#pragma endregion
	...
	Camera camera;
	camera.Initialize();
	camera.translation_ = Vector3(0.0f, 2.0f, -10.0f);

	uint32_t ON_DebugCamera = false;
	uint32_t OFF_DebugCamera = true;
...
#pragma region 被写体の更新
		// cube
		worldTransform.rotation_.y += 0.005f;
		worldTransform.UpdateMatrix();

		WT_terrain.UpdateMatrix();
#pragma endregion

#pragma region デバックカメラ

		if (Input::GetInstance()->TriggerKey(DIK_0)) {
			ON_DebugCamera = (ON_DebugCamera == false);
			OFF_DebugCamera = (ON_DebugCamera == false);
		}
		if (ON_DebugCamera) {
#pragma region 回転
			if (Input::GetInstance()->PushKey(DIK_LEFT)) {
				camera.rotation_.y -= 0.005f;
			}
...
#pragma endregion
			...
			if (Input::GetInstance()->TriggerKey(DIK_R)) {
				...
				camera.translation_.x = 0.0f;
				camera.translation_.y = 2.5f;
...
#pragma region 3Dモデル描画
		...
		M_terrain->Draw(WT_terrain, camera);
		Model::PostDraw();
